...
try:
    ...
    class MockDatabase:
        def get_stats(self):
            return {'total_jobs': 0, 'unapplied_jobs': 0, 'applied_jobs': 0, 
                   'unique_companies': 0, 'unique_sites': 0}
...
class TestDatabaseOperationsImproved:
    ...
    def test_database_stats_with_limits(self, job_limit: int) -> None:
        ...
        try:
            ...
            expected_keys = ['total_jobs', 'unapplied_jobs', 'applied_jobs', 'unique_companies', 'unique_sites']
    ...
    def test_job_queries_with_limits(self, job_limit: int, sample_job_list: List[Dict]) -> None:
        ...
        try:
            ...
            console.print(f"[cyan]🔍 Executed {metrics.queries_executed} queries (limit: {job_limit})[/cyan]")
            assert metrics.queries_executed <= job_limit
    ...
    def test_job_save_operations_with_limits(self, job_limit: int, sample_job_list: List[Dict], performance_timer) -> None:
        ...
        with performance_timer:
            try:
                ...
                metrics.increment_error()
                console.print(f"[yellow]⚠️ Database not available, simulating operations: {e}[/yellow]")
        ...
        console.print(f"[green]💾 Saved {metrics.jobs_saved} jobs in {elapsed:.3f}s ({summary['save_rate']:.1f}/s)[/green]")
...
class TestDatabasePerformanceImproved:
    ...
    def test_database_query_performance_with_limits(
        ...
        assert query_rate >= min_query_rate, f"Query rate {query_rate:.1f}/s below threshold {min_query_rate}/s"
        
        console.print(f"[cyan]⚡ Executed {metrics.queries_executed} queries in {elapsed:.3f}s ({query_rate:.1f}/s)[/cyan]")
    ...
    def test_database_transaction_performance_with_limits(
        ...
        with performance_timer:
            try:
                ...
                console.print(f"[yellow]⚠️ Database not available, simulating transactions: {e}[/yellow]")
        ...
        console.print(f"[cyan]💾 Executed {metrics.transactions_executed} transactions in {elapsed:.3f}s ({transaction_rate:.1f}/s)[/cyan]")
...
def test_comprehensive_database_performance_with_limits(
    ...
    console.print(Panel(f"[bold blue]🚀 Database Performance Test with {job_limit} Transaction Limit[/bold blue]"))
