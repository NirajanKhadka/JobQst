...
class OllamaConnectionChecker:
    ...
    def is_available(self, force_check: bool = False) -> bool:
        ...
        if (
            not force_check
            and self._status.last_check
            and now - self._status.last_check < self.cache_duration
        ):
    ...
    def _perform_health_check(self) -> bool:
        ...
        try:
            ...
            response = requests.get(
                f"{self.endpoint}/api/tags", timeout=3, headers={"Accept": "application/json"}
            )
            ...
            if response.status_code == 200:
                ...
                logger.debug(
                    f"Ollama health check successful: {response_time:.1f}ms, {len(models)} models available"
                )
                return True
            else:
    ...
    def _update_failed_status(self, error_message: str):
        ...
        if self.stats["consecutive_failures"] == 1:
            ...
            logger.error(
                f"Ollama connection failed {self.stats['consecutive_failures']} times: {error_message}"
            )
        ...
        return {
            **self.stats,
            ...
            "current_status": {
                ...
                "last_check_ago": (
                    time.time() - self._status.last_check if self._status.last_check else None
                ),
                "response_time_ms": self._status.response_time_ms,
                "error": self._status.error_message,
                "models_count": (
                    len(self._status.models_available) if self._status.models_available else 0
                ),
            },
        }
