...
class WorkerStatusTracker:
    ...
    def __init__(self):
        ...
        self.ai_service_status = {
            ...
            "available_models": [],
            "analysis_method_distribution": {"ai": 0, "Improved_rule_based": 0, "fallback": 0},
    ...
    def get_processing_statistics(self, worker_name: str) -> Dict[str, Any]:
        ...
        success_rate = (
            (worker.processed_count / total_attempts * 100) if total_attempts > 0 else 0.0
        )
    ...
    def update_worker_resources(
        self, worker_name: str, cpu_usage: float, memory_usage: Dict[str, Any], uptime: str
    ) -> bool:
        """
        ...
        try:
            ...
            worker.resource_usage = {"cpu": cpu_usage, "memory": memory_usage.get("percent", 0.0)}
    ...
    def cleanup_old_workers(self, max_age_hours: int = 24) -> int:
        ...
        for worker_name, worker in self.worker_states.items():
            if (
                worker.start_time
                and worker.start_time < cutoff_time
                and worker.worker_state == WorkerState.STOPPED
            ):
        ...
        try:
            ...
            if self.ollama_checker:
                ...
                if self.ai_service_status["ollama_available"]:
                    ...
                    self.ai_service_status["available_models"] = (
                        self.ollama_checker.get_available_models()
                    )
    ...
    def get_ai_service_health_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "primary_analysis_method": (
                max(
                    ai_status["analysis_method_distribution"],
                    key=ai_status["analysis_method_distribution"].get,
                )
                if total_analyses > 0
                else "none"
            ),
        ...
        try:
            ...
            if ai_health["health_status"] == "healthy":
                self.update_worker_task(
                    worker_name,
                    f"Monitoring AI services - {ai_health['available_models_count']} models available",
                )
            elif ai_health["health_status"] == "degraded":
                self.update_worker_task(
                    worker_name,
                    f"AI services degraded - {ai_health['consecutive_failures']} recent failures",
                )
            else:
                self.set_worker_error(
                    worker_name,
                    f"AI services unavailable - {ai_health['consecutive_failures']} consecutive failures",
                )
