...
class MultiAgentOrchestrator:
    ...
    async def start_multi_agent_system(self, 
                                     ...
                                     run_gmail_monitor: bool = True,
                                     enhance_jobs: bool = True) -> None:
        """
        ...
        console.print(f"[cyan]Gmail Monitoring: {'✅ Enabled' if run_gmail_monitor else '❌ Disabled'}[/cyan]")
    ...
    async def _run_sequential_mode(self, max_applications: int, enhance_jobs: bool, run_gmail_monitor: bool) -> None:
        ...
        if run_gmail_monitor:
            ...
            console.print("[yellow]Gmail monitor will run for 10 minutes to check for confirmations[/yellow]")
    ...
    async def _run_parallel_mode(self, max_applications: int, enhance_jobs: bool, run_gmail_monitor: bool) -> None:
    ...
    async def _run_application_agent(self, max_applications: int, enhance_jobs: bool) -> List:
        ...
        try:
            console.print(f"[cyan]🎯 Starting application agent for {max_applications} jobs...[/cyan]")
            ...
            console.print(f"[green]✅ Application agent completed: {len(results)} jobs processed[/green]")
            return results
    ...
    async def _run_gmail_monitor(self, duration_minutes: int = 10) -> None:
        ...
        try:
            console.print(f"[cyan]📧 Starting Gmail monitor for {duration_minutes} minutes...[/cyan]")
            
            monitor = BackgroundGmailMonitor(self.profile_name)
            
            # Create a task that will run for the specified duration
            monitor_task = asyncio.create_task(monitor.start_monitoring(check_interval=self.gmail_check_interval))
            ...
            console.print(f"[green]✅ Gmail monitor completed: {monitor_stats.get('confirmations_found', 0)} confirmations found[/green]")
    ...
    def _show_application_status_summary(self) -> None:
        ...
        try:
            ...
            status_table.add_row("Applied", str(stats.get('applied_jobs', 0)), "Confirmed via Gmail")
            status_table.add_row("Pending Verification", "?", "Waiting for Gmail confirmation")
            status_table.add_row("Failed", str(self.stats["applications_failed"]), "Application failed")
            status_table.add_row("Unapplied", str(stats.get('unapplied_jobs', 0)), "Not yet processed")
    ...
    def _display_final_statistics(self) -> None:
        ...
        session_table.add_row("Session Duration", f"{self.stats['total_runtime']:.1f} seconds ({runtime_hours:.2f} hours)")
...
async def start_job_application_system(profile_name: str = "Nirajan",
                                     max_applications: int = 10,
                                     run_gmail_monitor: bool = True) -> None:
    """
