...
"""
iCIMS ATS Submitter
Handles job applications to iCIMS-based systems.
"""
...
class ICIMSSubmitter(BaseSubmitter):
    ...
    def get_field_mapping(self) -> Dict[str, Dict[str, str]]:
        """Get iCIMS-specific field mapping."""
        return {
            'personal_info': {
    ...
    def _submit_to_ats(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Submit data to iCIMS ATS."""
        return {
            'success': True,
            'application_id': f'icims_{int(time.time())}',
