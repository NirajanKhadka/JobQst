#!/usr/bin/env python3
"""
Greenhouse ATS Submitter
Handles job applications to Greenhouse-based systems.
"""
...
class GreenhouseSubmitter(BaseSubmitter):
    ...
    def get_field_mapping(self) -> Dict[str, Dict[str, str]]:
        """Get Greenhouse-specific field mapping."""
        return {
            'personal_info': {
    ...
    def detect_application_form(self, page_data: Dict[str, Any]) -> Dict[str, Any]:
        """Detect Greenhouse application form."""
        return {
            'detected': True,
    ...
    def _submit_to_ats(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Submit data to Greenhouse ATS."""
        return {
            'success': True,
            'application_id': f'greenhouse_{int(time.time())}',
