...
class ApplicationFlowOptimizer:
    ...
    def predict_success_rate(self, job_data: Dict[str, Any]) -> float:
        ...
        similar_jobs = [p for p in self.application_patterns 
                       if p['job_type'] == job_data.get('job_type')]
    ...
    def get_optimization_suggestions(self, job_data: Dict[str, Any]) -> List[str]:
        ...
        job_type = job_data.get('job_type')
        if job_type:
            job_type_patterns = [p for p in self.application_patterns 
                               if p['job_type'] == job_type]
            if job_type_patterns:
                success_rate = sum(1 for p in job_type_patterns if p['success']) / len(job_type_patterns)
                if success_rate < 0.3:
                    suggestions.append(f"Low success rate for {job_type} jobs. Consider improving resume.")
                    
        # Analyze ATS patterns
        ats_type = job_data.get('ats_type')
        if ats_type:
            ats_patterns = [p for p in self.application_patterns 
                          if p['ats_type'] == ats_type]
            if ats_patterns:
                ats_success_rate = sum(1 for p in ats_patterns if p['success']) / len(ats_patterns)
                if ats_success_rate < 0.4:
                    suggestions.append(f"Low success rate with {ats_type} ATS. Consider manual application.")
    ...
    def optimize_application_timing(self) -> Dict[str, Any]:
        ...
        # Find best hour
        best_hour = max(timing_data.keys(), 
                       key=lambda h: timing_data[h]['success'] / timing_data[h]['total'] 
                       if timing_data[h]['total'] > 0 else 0)
        
        # Find best day
        best_day = max(timing_data.keys(), 
                      key=lambda d: timing_data[d]['success'] / timing_data[d]['total'] 
                      if timing_data[d]['total'] > 0 else 0)
...
def create_flow_optimizer(profile_name: str, config: Optional[Dict[str, Any]] = None) -> ApplicationFlowOptimizer:
    """
