...
class JobLanguageDetector:
    ...
    def __init__(self, english_threshold: float = 0.6, french_threshold: float = 0.4):
        ...
        self.french_indicators = [
            'emploi', 'travail', 'poste', 'carrière', 'candidature',
            'expérience', 'compétences', 'formation', 'diplôme',
            'salaire', 'horaire', 'temps partiel', 'temps plein',
            'entreprise', 'société', 'équipe', 'département',
            'responsabilités', 'qualifications', 'exigences',
            'bilingue', 'français', 'anglais', 'québec', 'montreal',
        ...
        self.english_indicators = [
            ...
            'responsibilities', 'salary', 'benefits', 'full-time',
            'part-time', 'remote', 'hybrid', 'on-site', 'company',
            'team', 'department', 'apply', 'application', 'resume'
        ]
    ...
    def _should_process_job(self, primary_lang: str, confidence: float, 
                           lang_probs: List[Dict], french_score: float, 
                           english_score: float) -> Tuple[bool, str]:
        """
        ...
        if french_prob >= self.french_threshold:
            return False, f"French probability too high: {french_prob:.2f} >= {self.french_threshold}"
    ...
    def filter_jobs_by_language(self, jobs: List[Dict]) -> Tuple[List[Dict], List[Dict]]:
        ...
        logger.info(f"Language filtering: {len(processable_jobs)} processable, {len(filtered_jobs)} filtered")
...
def test_language_detector():
    ...
    for i, job in enumerate(test_jobs, 1):
        ...
        if result['languages']:
            lang_list = [f"{lang['language']}: {lang['probability']:.2f}" for lang in result['languages']]
