...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("logs/ai_service.log", mode="a")],
)
...
class AIServiceLogger:
    ...
    def log_connection_check(
        ...
        if success:
            ...
            logger.warning(
                f"❌ {service_name} connection failed ({duration_ms:.1f}ms): {error_message}"
            )
    ...
    def log_analysis_attempt(
        ...
        event_metadata.update(
            {"analysis_method": analysis_method, "compatibility_score": compatibility_score}
        )
        ...
        if success:
            ...
            logger.info(
                f"🎯 {service_name} analysis successful ({analysis_method}, {duration_ms:.1f}ms, {score_text})"
            )
        else:
            logger.error(
                f"💥 {service_name} analysis failed ({analysis_method}, {duration_ms:.1f}ms): {error_message}"
            )
    ...
    def log_fallback_usage(
        ...
        event_metadata = metadata or {}
        event_metadata.update(
            {"from_method": from_method, "to_method": to_method, "reason": reason}
        )
    ...
    def log_error(
        ...
        event_metadata = metadata or {}
        event_metadata.update(
            {"error_type": error_type, "traceback": traceback.format_exc() if exception else None}
        )
    ...
    def _record_event(self, event: AIServiceEvent):
        ...
        if event.duration_ms:
            ...
            if total_with_duration > 0:
                avg_duration = (
                    sum(e.duration_ms for e in self.events if e.duration_ms) / total_with_duration
                )
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        if self.metrics["analysis_attempts"] > 0:
            success_rate = (
                self.metrics["successful_analyses"] / self.metrics["analysis_attempts"]
            ) * 100
        ...
        return {
            **self.metrics,
            ...
            "fallback_rate": round(
                (self.metrics["fallback_uses"] / max(self.metrics["analysis_attempts"], 1)) * 100, 1
            ),
            "error_rate": round(
                (self.metrics["errors"] / max(self.metrics["total_events"], 1)) * 100, 1
            ),
        }
    ...
    def get_error_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        recent_errors = [
            ...
            and datetime.fromisoformat(e.timestamp).timestamp() > cutoff_time
        ]
        ...
        return {
            ...
            "error_types": {error_type: len(errors) for error_type, errors in error_types.items()},
            "most_common_error": (
                max(error_types.keys(), key=lambda k: len(error_types[k])) if error_types else None
            ),
            "recent_errors": [
                {
    ...
    def get_user_friendly_status(self) -> Dict[str, Any]:
        ...
        if metrics["fallback_rate"] > 50:
            recommendations.append(
                "Frequently falling back to rule-based analysis - check AI service connectivity"
            )
        if metrics["average_response_time"] > 30000:
            recommendations.append(
                "AI services are responding slowly - consider optimizing configuration"
            )
...
def log_ai_connection_check(
    service_name: str, success: bool, duration_ms: float, error_message: Optional[str] = None
):
    """Log AI service connection check."""
...
def log_ai_analysis(
    ...
    logger_instance = get_ai_service_logger()
    logger_instance.log_analysis_attempt(
        service_name, success, duration_ms, analysis_method, compatibility_score, error_message
    )
...
def log_ai_error(
    service_name: str, error_type: str, error_message: str, exception: Optional[Exception] = None
):
