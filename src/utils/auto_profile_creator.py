...
class AutoProfileCreator:
    ...
    def _load_profile_template(self) -> Dict[str, Any]:
        ...
        # Fallback template
        return {"name": "", "email": "", "skills": [], "keywords": [], "experience_level": "entry"}
    ...
    def _create_profile_from_template(
        self, profile_name: str, analysis: Dict[str, Any], resume_path: str
    ) -> Dict[str, Any]:
    ...
    def _set_nested_field(
        self, profile: Dict, field_path: str, analysis: Dict[str, Any], resume_path: str
    ):
    ...
    def _extract_links(self, text: str) -> Dict[str, str]:
        ...
        for pattern in portfolio_patterns:
            ...
            if (
                match
                and "linkedin" not in match.group().lower()
                and "github" not in match.group().lower()
            ):
    ...
    def _determine_experience_level(self, text_lower: str) -> str:
        ...
        if any(
            word in text_lower for word in ["senior", "lead", "principal", "director", "manager"]
        ):
            return "senior"
        elif any(
            word in text_lower
            for word in ["mid-level", "intermediate", "3 years", "4 years", "5 years"]
        ):
            return "mid"
        elif any(
            word in text_lower for word in ["junior", "entry", "intern", "graduate", "new grad"]
        ):
            return "entry"
        else:
            return "mid"  # Default
