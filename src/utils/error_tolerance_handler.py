            ...
            def wrapper(*args, **kwargs) -> Any:
                ...
                for attempt in range(retries + 1):
                    try:
                        ...
                        if attempt > 0:
                            console.print(
                                f"[green]✅ {operation_name} succeeded on attempt {attempt + 1}[/green]"
                            )
                        return result
                        ...
                        if attempt < retries:
                            ...
                            console.print(
                                f"[yellow]⚠️ {operation_name} failed (attempt {attempt + 1}/{retries + 1}): {e}[/yellow]"
                            )
                            console.print(f"[cyan]🔄 Retrying in {delay:.1f} seconds...[/cyan]")
                            time.sleep(delay)
                        else:
                            console.print(
                                f"[red]❌ {operation_name} failed after {retries + 1} attempts[/red]"
                            )
        ...
        def decorator(func: Callable) -> Callable:
            ...
            def wrapper(*args, **kwargs) -> Any:
                try:
                    ...
                    console.print(
                        f"[yellow]⚠️ {operation_name} failed, trying fallback: {e}[/yellow]"
                    )
                    try:
                        ...
                        console.print(
                            f"[red]❌ {operation_name} fallback also failed: {fallback_error}[/red]"
                        )
            ...
            def wrapper(*args, **kwargs) -> Any:
                ...
                if state["state"] == "open":
                    if (
                        ...
                        console.print(
                            f"[yellow]🔄 Circuit breaker for {operation_name} moving to half-open[/yellow]"
                        )
                    else:
                ...
                try:
                    ...
                    if state["failures"] > 0:
                        console.print(
                            f"[green]✅ Circuit breaker for {operation_name} reset[/green]"
                        )
                    state["failures"] = 0
                    state["state"] = "closed"
                    return result
                    ...
                    if state["failures"] >= failure_threshold:
                        state["state"] = "open"
                        console.print(
                            f"[red]🚫 Circuit breaker opened for {operation_name} after {failure_threshold} failures[/red]"
                        )
...
class SystemHealthMonitor:
    ...
    def register_health_check(
        self, name: str, check_func: Callable, critical: bool = False
    ) -> None:
    ...
    def get_health_summary(self) -> Dict:
        ...
        summary = {
            ...
            "healthy_checks": len(
                [c for c in latest["checks"].values() if c["status"] == "healthy"]
            ),
...
_health_checks_registered = False
...
# Register default health checks only once
if not _health_checks_registered:
    health_monitor.register_health_check(
        "database_connection", check_database_connection, critical=True
    )
    ...
    _health_checks_registered = True
