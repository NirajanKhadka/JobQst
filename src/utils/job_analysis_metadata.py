...
class JobAnalysisMetadataHandler:
    ...
    def update_job_with_metadata(self, job_data: Dict[str, Any], 
    ...
    def get_analysis_method_distribution(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'primary_method': primary_method if self.analysis_method_stats[primary_method] > 0 else None
        }
    ...
    def filter_jobs_by_analysis_method(self, jobs: List[Dict[str, Any]], 
        ...
        return [
            job for job in jobs 
            if job.get('analysis_method') == method
        ]
    
    def get_jobs_needing_reanalysis(self, jobs: List[Dict[str, Any]], 
                                   prefer_method: str = 'mistral_7b') -> List[Dict[str, Any]]:
        """
    ...
    def _generate_recommendations(self, distribution: Dict[str, Any], 
        ...
        for method, stats in performance['methods'].items():
            if stats['average_duration_ms'] > 30000:  # 30 seconds
                recommendations.append(f"{method} is slow (avg: {stats['average_duration_ms']:.0f}ms)")
...
def update_job_with_analysis_metadata(job_data: Dict[str, Any], 
                                    analysis_result: Dict[str, Any]) -> Dict[str, Any]:
