...
class OptimizedTwoStageProcessor:
    ...
    def __init__(
        ...
        if self.cpu_workers is None:
            ...
            self.cpu_workers = min(10, max(4, self.hardware_config.optimal_batch_size))
    ...
    def _ensure_base_processor(self):
        ...
        if self._base_processor is None:
            ...
            self._base_processor = TwoStageJobProcessor(
                self.user_profile, self.cpu_workers, self.max_concurrent_stage2
            )

            # Initialize batch processor if Stage 2 is available
            if (
                hasattr(self._base_processor, "stage2_processor")
                and self._base_processor.stage2_processor is not None
            ):
                try:
                    ...
                    console.print("[yellow]⚠️ Falling back to individual processing[/yellow]")

            # Display optimization info
            console.print("[bold blue]🚀 Optimized Two-Stage Processor Initialized" "[/bold blue]")
            console.print(f"[cyan]   Hardware: {self.hardware_config.device_name}[/cyan]")
            console.print(f"[cyan]   Stage 1 Workers: {self.cpu_workers}[/cyan]")
            console.print(f"[cyan]   Stage 2 Concurrency: {self.max_concurrent_stage2}[/cyan]")
    ...
    async def process_jobs(self, jobs: List[Dict[str, Any]]) -> List[TwoStageResult]:
        ...
        console.print(f"[cyan]   Phase 1 threshold: {config.phase1_threshold:.2f}[/cyan]")
        console.print(f"[cyan]   Max Phase 2 jobs: {config.max_phase2_jobs}[/cyan]")
        ...
        if self._base_processor.stage2_processor is None:
            ...
            final_results = self._create_stage1_only_results(jobs, stage1_results)
        ...
        console.print("\n[bold green]✅ Processing completed in " f"{total_time:.1f}s[/bold green]")
        console.print(f"[green]   Jobs processed: {len(final_results)}[/green]")
        console.print(f"[green]   Stage 2 jobs: {len(passed_jobs)}[/green]")
        console.print("[green]   Performance: " f"{len(jobs)/total_time:.1f} jobs/second[/green]")
    ...
    async def _process_with_batch_optimization(
        ...
        console.print(
            f"[cyan]🚀 Using batch processing (" f"{config.phase2_batch_size} jobs/batch)[/cyan]"
        )
        ...
        try:
            # Use batch processor
            stage2_batch_results = await self.batch_processor.process_jobs_batch(
                jobs_with_stage1,
            ...
            for idx, (job, s1) in enumerate(zip(jobs, stage1_results)):
                if any(original_idx == idx for _, _, original_idx in passed_jobs):
            ...
            console.print(
                "[red]❌ Batch processing failed, falling back to " "individual processing[/red]"
            )
            # Fall back to individual processing
            return await self._process_with_individual_workers(jobs, stage1_results, passed_jobs)
    ...
    async def _process_with_individual_workers(
        ...
        for job, s1, original_idx in passed_jobs:
            t = asyncio.create_task(
                process_single_wrapped(original_idx, job, s1), name=f"stage2_job_{original_idx}"
            )
            tasks.append(t)
        ...
        with Progress(
            ...
            task = progress.add_task("Stage 2 Processing...", total=len(tasks))
            ...
            for fut in asyncio.as_completed(tasks):
                try:
                    ...
                    logger.error(f"Individual processing task failed: {e}")
                    progress.update(task, advance=1)
    ...
    def _combine_results(
        self, job: Dict[str, Any], stage1: Stage1Result, stage2: Stage2Result, job_index: int
    ) -> TwoStageResult:
    ...
    def _create_stage1_only_result(
        self, job: Dict[str, Any], stage1: Stage1Result, job_index: int
    ) -> TwoStageResult:
    ...
    def _create_stage1_only_result(
        ...
        return TwoStageResult(
            ...
            recommendation=("review" if s1.basic_compatibility >= 0.4 else "skip"),
            total_processing_time=s1.processing_time,
            stages_completed=1,
            processing_method="stage1_only",
        )

    def _convert_batch_result_to_stage2(self, batch_result: Dict[str, Any]) -> Stage2Result:
        ...
        return Stage2Result(
            ...
            semantic_compatibility=batch_result.get("semantic_compatibility", 0.5),
            job_sentiment=batch_result.get("job_sentiment", "neutral"),
            skill_embeddings=batch_result.get("skill_embeddings"),
            contextual_understanding=batch_result.get("contextual_understanding", ""),
...
def create_optimized_processor(
    user_profile: Dict[str, Any], force_individual: bool = False, **kwargs
) -> TwoStageJobProcessor:
    """
    ...
    if force_individual:
        ...
        console.print("[yellow]⚠️ Using individual processing mode " "(compatibility)[/yellow]")
    ...
    try:
        ...
        console.print("[red]❌ Optimization failed, falling back to " "original processor[/red]")
        return TwoStageJobProcessor(user_profile, **kwargs)
