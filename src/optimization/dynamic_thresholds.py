...
class DynamicThresholdManager:
    ...
    def calculate_optimal_thresholds(
        self, total_jobs: int, user_preferences: Dict[str, float] = None
    ) -> ThresholdConfig:
        ...
        max_phase2_jobs = min(int(total_jobs * max_phase2_ratio), self._get_max_phase2_capacity())
    ...
    def should_enable_phase2(self, total_jobs: int) -> bool:
        ...
        if (
            ...
            and "PyTorch not available" in self.hardware_config.device_name
        ):
...
def get_optimal_processing_config(
    total_jobs: int, user_preferences: Dict[str, float] = None
