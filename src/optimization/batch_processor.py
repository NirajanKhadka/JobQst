...
class BatchProcessor:
    ...
    async def process_jobs_batch(
        self, jobs_with_stage1: List[Tuple[Dict[str, Any], Any]], config: Optional[Dict] = None
    ) -> List[Dict[str, Any]]:
    ...
    async def _process_batch_gpu(
        self, batch: List[Tuple[Dict[str, Any], Any]]
    ) -> List[Dict[str, Any]]:
        ...
        def process_batch_sync():
            ...
            inputs = self.tokenizer(
                job_descriptions, padding=True, truncation=True, max_length=512, return_tensors="pt"
            ).to(self.device)
            ...
            for idx, (job_data, stage1_result) in enumerate(batch_data):
                ...
                result = self._create_stage2_result(job_data, stage1_result, embedding)
    ...
    def _create_stage2_result(
        self, job_data: Dict[str, Any], stage1_result: Any, embedding: np.ndarray
    ) -> Dict[str, Any]:
    ...
    def _create_stage2_result_cpu(
        self, job_data: Dict[str, Any], stage1_result: Any
    ) -> Dict[str, Any]:
    ...
    def _extract_skills_from_embedding(self, embedding: np.ndarray, description: str) -> List[str]:
    ...
    def _calculate_semantic_compatibility(
        self, embedding: np.ndarray, stage1_result: Any, skills: List[str]
    ) -> float:
