...
class CLIOrchestrator:
    ...
    def run(self, args) -> int:
        self.logger.info(
            f"Starting CLI session for profile: {self.profile.get('profile_name', 'Unknown')}"
        )
        while True:
...
class MainMenu:
        ...
        try:
            ...
            if job_count > 0:
                ...
                if unprocessed_count > 0:
                    console.print(
                        f"[yellow]⚠️ {unprocessed_count} unprocessed jobs " f"detected[/yellow]"
                    )
                ...
                if recent_jobs:
                    titles = [job.get("title", "Unknown")[:20] + "..." for job in recent_jobs]
                    console.print(f"[dim]� Recent: {', '.join(titles)}[/dim]")
        except Exception:
    ...
    async def _auto_process_jobs_action(self, args) -> None:
        ...
        try:
            ...
            # Use UnifiedJobProcessingPipeline via compatibility layer
            from src.pipeline.pipeline_migration import create_optimized_processor

            console.print(
                f"[cyan]🔄 Processing {len(jobs_to_process)} existing jobs with ultra-fast method...[/cyan]"
            )
            ...
            if processed_jobs:
                ...
                if processed_jobs:
                    ...
                    for i, result in enumerate(processed_jobs[:3], 1):
                        ...
                        try:
                            if (
                                result.stage1
                                and hasattr(result.stage1, "title")
                                and result.stage1.title
                            ):
                            ...
                            if (
                                result.stage1
                                and hasattr(result.stage1, "company")
                                and result.stage1.company
                            ):
                ...
                console.print(
                    "[cyan]💡 Try a different processing method or check job data quality[/cyan]"
                )
        ...
        try:
            ...
            try:
                while True:
                    ...
                    if new_resume_files:
                        console.print(
                            f"[green]📄 Processing resume: {new_resume_files[0].name}[/green]"
                        )
                        ...
                        if success:
                            console.print("[green]✅ Resume processed successfully![/green]")
                            console.print(
                                "[cyan]💡 Keywords and skills extracted automatically[/cyan]"
                            )
                            break
                        else:
                            console.print("[red]❌ Resume processing failed![/red]")
                        ...
                        console.print(
                            "[yellow]⚠️ No resume files found. Please copy your resume to the folder.[/yellow]"
                        )
