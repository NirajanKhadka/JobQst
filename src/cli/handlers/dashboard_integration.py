...
class DashboardScrapingIntegration:
    ...
    def run_scraping_with_dashboard_updates(
        self, mode: str = "simple", jobs: int = 20, sites: Optional[List[str]] = None, **kwargs
    ) -> bool:
        ...
        try:
            ...
            success = self._run_scraping_with_monitoring(operation_id, mode, jobs, sites, **kwargs)
            ...
            self._notify_dashboard(
                "scraping_error", {"operation_id": operation_id, "error": str(e)}
            )

            raise

    def _run_scraping_with_monitoring(
        self, operation_id: str, mode: str, jobs: int, sites: Optional[List[str]], **kwargs
    ) -> bool:
    ...
    def _monitored_pipeline_execution(
        self, original_method: Callable, monitor: "PipelineMonitor", mode: str, jobs: int
    ) -> bool:
    ...
    def get_performance_summary(self) -> Dict:
        ...
        avg_performance = (
            sum(metrics.get("jobs_per_second", 0) for metrics in self.performance_metrics.values())
            / total_ops
        )
...
class UnifiedScrapingOrchestrator(ScrapingOrchestrator):
    ...
    def run_scraping_with_dashboard(self, *args, **kwargs):
        ...
        try:
            result = self.dashboard_integration.run_scraping_with_dashboard_updates(*args, **kwargs)
