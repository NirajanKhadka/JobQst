...
class EnhancedJobDescriptionScraper:
    ...
    def _extract_experience_requirements(self, description: str) -> Dict[str, Any]:
        ...
        for sentence in sentences:
            ...
            if any(keyword in sentence_lower for keyword in ['experience', 'years', 'level', 'senior', 'junior', 'entry']):
                experience_sentences.append(sentence.strip())
                
        experience_info['experience_text'] = ' '.join(experience_sentences[:3])  # First 3 relevant sentences
    ...
    def _extract_skills_and_requirements(self, description: str) -> Dict[str, Any]:
        ...
        soft_skills = [
            ...
            'creativity', 'attention to detail', 'multitasking', 'collaboration'
        ]
        ...
        for sentence in sentences:
            sentence_lower = sentence.lower()
            if any(keyword in sentence_lower for keyword in ['require', 'must', 'should', 'prefer', 'qualification']):
    ...
    async def _extract_metadata(self, page: Page) -> Dict[str, Any]:
        ...
        try:
            ...
            if any(term in page_content.lower() for term in ['remote', 'work from home', 'wfh', 'telecommute']):
                metadata['remote_work'] = True
                
            # Check for relocation assistance
            if any(term in page_content.lower() for term in ['relocation', 'relocate', 'moving assistance']):
