...
class ExternalJobDescriptionScraper(ImprovedJobDescriptionScraper):
    ...
    async def scrape_external_jobs_parallel(self, job_urls: List[str]) -> List[Dict[str, Any]]:
        ...
        console.print(
            f"[cyan]📋 {len(job_urls)} jobs to scrape with {self.num_workers} workers[/cyan]"
        )
        ...
        if len(external_urls) != len(job_urls):
            console.print(
                f"[yellow]⚠️ Filtered out {len(job_urls) - len(external_urls)} Eluta URLs[/yellow]"
            )
            ...
            try:
                ...
                with Progress() as progress:
                    task = progress.add_task(
                        "[green]Scraping external job descriptions...", total=len(external_urls)
                    )
        ...
        self.stats["jobs_per_second"] = (
            len(results) / self.stats["processing_time"] if self.stats["processing_time"] > 0 else 0
        )
    ...
    async def _external_worker_loop(
        self, browser, worker_id: int, job_queue: asyncio.Queue, progress: Progress, task: TaskID
    ) -> List[Dict[str, Any]]:
        """
            ...
            while True:
                try:
                    ...
                    if job_data and job_data.get("description"):
                        ...
                        console.print(
                            f"[green]✅ Worker {worker_id}: Scraped {job_data.get('title', 'Unknown')[:30]}...[/green]"
                        )
                    else:
                        console.print(
                            f"[yellow]⚠️ Worker {worker_id}: Failed to scrape {job_url[:50]}...[/yellow]"
                        )
        ...
        console.print(
            f"[dim]🏁 Worker {worker_id} finished: {len(worker_results)} jobs scraped[/dim]"
        )
...
async def test_external_scraper():
    ...
    for result in results:
        console.print(
            f"[green]- {result.get('title', 'Unknown')} at {result.get('company', 'Unknown')}[/green]"
        )
