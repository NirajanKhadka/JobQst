...
class ScrapingMetrics:
    ...
    def finalize(self):
        ...
        if self.unique_jobs_added > 0:
            total_quality = sum(
                p.get("avg_quality_score", 0) * p.get("total_jobs_processed", 0)
                for p in self.keyword_performance.values()
            )
            self.data_quality_score = total_quality / self.unique_jobs_added

    def update_batch_metrics(
        self, keywords: List[str], raw_jobs: List[Dict], processed_jobs: List[Dict]
    ):
