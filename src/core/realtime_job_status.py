...
class RealTimeJobStatusManager:
    ...
    def get_job_counts(self, profile_name: str, force_refresh: bool = False) -> JobCounts:
        ...
        with self._lock:
            ...
            self._log_activity(
                "job_counts_updated",
                profile_name,
                {"total_jobs": counts.total_jobs, "processed_jobs": counts.processed_jobs},
            )
    ...
    def mark_job_processed(self, profile_name: str, job_id: str, job_title: str = "") -> None:
        ...
        with self._lock:
            ...
            self._log_activity(
                "job_processed", profile_name, {"job_id": job_id, "job_title": job_title}
            )
    ...
    def get_recent_activity(
        self, profile_name: Optional[str] = None, limit: int = 10
    ) -> List[Dict[str, Any]]:
        """
    ...
    def cleanup_old_data(self, max_age_hours: int = 24) -> None:
        ...
        with self._lock:
            ...
            expired_profiles = [
                profile
                for profile, timestamp in self._cache_timestamps.items()
    ...
    def _fetch_job_counts_from_db(self, profile_name: str) -> JobCounts:
        ...
        try:
            ...
            return JobCounts(
                error_jobs=1, last_updated=datetime.now().isoformat()  # Indicate there's an error
            )
    ...
    def _calculate_processing_rate(self, profile_name: str) -> float:
        ...
        recent_activities = [
            a
            for a in self._activity_log
            if (
                a.get("profile_name") == profile_name
                and a.get("action") == "job_processed"
                and datetime.fromisoformat(a.get("timestamp", "")) > cutoff_time
            )
        ]
