...
class JobRelevanceFilter:
    ...
    def _extract_target_roles(self) -> Set[str]:
        ...
        for keyword in keywords:
            ...
            if any(role in keyword_lower for role in ["developer", "engineer"]):
                if not any(
                    excluded in keyword_lower
                    for excluded in ["backend", "full stack", "fullstack", "front end", "frontend"]
                ):
    ...
    def is_relevant_job(self, job: Dict[str, Any]) -> bool:
        ...
        if any(term in title for term in ["software", "python", "application", "web"]):
            if any(term in title for term in ["developer", "engineer"]):
                ...
                if not any(
                    excluded in title
                    for excluded in ["backend", "back-end", "full stack", "fullstack"]
                ):
                    return True
    ...
    def filter_jobs(self, jobs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        console.print(
            f"[cyan]🎯 Job filtering: {len(relevant_jobs)} relevant, {filtered_count} filtered out[/cyan]"
        )
...
def filter_jobs_by_relevance(
    jobs: List[Dict[str, Any]], profile: Dict[str, Any]
) -> List[Dict[str, Any]]:
...
def filter_entry_level_jobs(
    jobs: List[Dict[str, Any]], include_entry_level: bool = True
) -> List[Dict[str, Any]]:
