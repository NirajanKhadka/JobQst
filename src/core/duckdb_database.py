...
class DuckDBJobDatabase:
    ...
    def __init__(self, db_path: str = "data/jobs_duckdb.db", profile_name: Optional[str] = None):
        """Initialize DuckDB connection with minimal schema."""
        self.profile_name = profile_name

        # Handle profile-specific paths
        if profile_name:
            # STANDARDIZE: Always use lowercase for database filenames
            profile_name_lower = profile_name.lower()
            profile_dir = Path(f"profiles/{profile_name}")
            
            # Primary path: lowercase version (standard)
            standard_path = profile_dir / f"{profile_name_lower}_duckdb.db"
            
            # Also check for case variations (legacy support)
            legacy_paths = [
                profile_dir / f"{profile_name}_duckdb.db",
                profile_dir / f"{profile_name.upper()}_duckdb.db",
            ]
            
            # Find existing database (prefer standard lowercase)
            existing_db = None
            if standard_path.exists():
                existing_db = standard_path
                logger.info(f"Found existing database: {standard_path}")
            else:
                # Check legacy paths
                for path in legacy_paths:
                    if path.exists():
                        existing_db = path
                        logger.info(f"Found existing database (legacy): {path}")
                        break
            
            # Use existing or create new with lowercase standard name
            self.db_path = str(existing_db) if existing_db else str(standard_path)
            self.db_file = Path(self.db_path)
        else:
            self.db_path = db_path
            self.db_file = Path(self.db_path)

        # Ensure directory exists (skip for special paths like :memory:)
    ...
    def _create_table(self):
        ...
        index_queries = [
            ...
            "CREATE INDEX IF NOT EXISTS idx_jobs_profile_name " "ON jobs(profile_name);",
    ...
    def _create_tracking_tables(self):
        ...
        tables = [notes_sql, interviews_sql, communications_sql, documents_sql, manual_review_sql]
        for sql in tables:
            try:
    ...
    def add_job(self, job_data) -> bool:
        ...
        try:
            ...
            insert_sql = f"INSERT INTO jobs ({columns}) " f"VALUES ({placeholders})"
            self.conn.execute(insert_sql, list(job_dict.values()))

            logger.debug(f"Added job: {job_dict['title']} " f"at {job_dict['company']}")
            return True
    ...
    def get_top_jobs(
        self, limit: int = 50, profile_name: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """Get top jobs with limit."""
        return self.get_jobs(profile_name=profile_name, limit=limit)

    def search_jobs(self, keyword: str, profile_name: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    def get_job_stats(self, profile_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get unapplied jobs count
            unapplied_query = f"""
            SELECT COUNT(*) FROM jobs {profile_filter}
            {'AND' if profile_filter else 'WHERE'} (application_status IS NULL OR application_status = 'discovered')
            """
            unapplied_result = self.conn.execute(unapplied_query, params).fetchone()
            unapplied_count = unapplied_result[0] if unapplied_result else 0
            
            # Get applied jobs count
            applied_query = f"""
            SELECT COUNT(*) FROM jobs {profile_filter}
            {'AND' if profile_filter else 'WHERE'} (application_status = 'applied' OR status = 'applied')
            """
            applied_result = self.conn.execute(applied_query, params).fetchone()
            ...
            return {
                ...
                "unapplied_jobs": unapplied_count,
    ...
    def _dict_to_minimal_dict(self, job_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(date_posted, str):
            try:
                ...
                logger.debug("Date parse failed: %s", str(e))
                date_posted = None
    ...
    def _job_data_to_minimal_dict(self, job_data: JobData) -> Dict[str, Any]:
        ...
        if not job_id:
            job_id = f"{job_data.company}_{job_data.title}_{job_data.location}".replace(
                " ", "_"
            ).lower()
        ...
        if isinstance(date_posted, str):
            try:
                ...
                logger.debug("Date parsing failed for '%s': %s", date_posted, str(e))
                date_posted = None
        elif isinstance(date_posted, datetime):
            date_posted = date_posted.date()
    ...
    def get_jobs_for_processing(
        self, limit: int = 10, profile_name: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    def update_job_processing_status(self, job_id: str, processing_data: Dict[str, Any]) -> bool:
    ...
    def update_job_analysis(self, job_id: str, analysis_data: Dict[str, Any]) -> bool:
    ...
    def get_job_by_url(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            result = self.conn.execute("SELECT * FROM jobs WHERE url = ? LIMIT 1", [url]).fetchone()
    ...
    def update_job_metadata(self, job_id: str, metadata: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not set_clauses:
                logger.debug(
                    f"No valid metadata fields to update "
    ...
    def clear_all_jobs(self, profile_name: Optional[str] = None) -> bool:
        """Clear all jobs from the database (useful for testing)."""
        try:
            self._ensure_connection()
            
            # If profile_name provided, delete only that profile's jobs
            if profile_name:
                self.conn.execute("DELETE FROM jobs WHERE profile_name = ?", [profile_name])
            elif self.profile_name:
                # Use instance profile if set
                self.conn.execute("DELETE FROM jobs WHERE profile_name = ?", [self.profile_name])
            else:
                # Delete all jobs if no profile specified
                self.conn.execute("DELETE FROM jobs")
            
            self.conn.commit()
            logger.debug(f"Cleared all jobs from database (profile: {profile_name or self.profile_name or 'all'})")
            return True
