...
class SmartJobDeduplicator:
    ...
    def is_duplicate_job(self, job: Dict[str, Any], existing_jobs: List[Dict[str, Any]]) -> Tuple[bool, str]:
        """
        ...
        title_company_key = f"{self.normalize_text(job_title)}|{self.normalize_company(job_company)}"
        ...
        for existing_job in existing_jobs:
            ...
            if (title_similarity >= 0.9 and company_similarity >= 0.8):
                return True, f"High similarity (T:{title_similarity:.2f}, C:{company_similarity:.2f})"
    ...
    def deduplicate_job_list(self, jobs: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], Dict[str, int]]:
        """
        ...
        for i, job in enumerate(jobs):
            ...
            if is_duplicate:
                ...
                if duplicate_count <= 5:
                    ...
                    console.print(f"[dim]🔄 Duplicate {duplicate_count}: {title}... at {company} ({reason})[/dim]")
            else:
                unique_jobs.append(job)
                self.add_job_to_tracking(job)
        
        # Show deduplication summary
        console.print(f"[green]✅ Deduplication complete: {len(unique_jobs)} unique, {duplicate_count} duplicates removed[/green]")
...
def smart_deduplicate_jobs(jobs: List[Dict[str, Any]], similarity_threshold: float = 0.85) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    """
