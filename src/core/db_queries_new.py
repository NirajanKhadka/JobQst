...
class DBQueries:
            ...
            if search_query:
                ...
                jobs = [
                    ...
                    if (search_term in job.get('title', '').lower() or
                        ...
                        search_term in job.get('summary', '').lower())
                ]
    ...
    def get_job_stats(self) -> Dict:
        ...
        try:
            ...
            total_jobs = len(jobs)
            applied_jobs = len([job for job in jobs if job.get('applied', False) or 
            ...
            failed_applications = len([
                job for job in jobs 
                if job.get('application_status') in ['rejected', 'failed']
            ])
            
            unapplied_jobs = len([
                job for job in jobs 
                if not job.get('applied', False) and 
                job.get('application_status') in [None, 'not_applied']
            ])
            
            manual_review_needed = len([
                job for job in jobs 
                if job.get('status') in ['pending_review', 'needs_review'] or
                job.get('application_status') == 'pending_review'
            ])
            ...
            recent_jobs = len([
                ...
                datetime.fromisoformat(job['created_at'].replace('Z', '+00:00')) > yesterday
            ])
