...
class DatabaseBackupManager:
    ...
    def restore_profile(
        ...
        try:
            # Find backup file
            backup_path = self._find_backup_file(
                profile_name, backup_timestamp, backup_type
            )
            ...
            if target_db.exists():
                emergency_backup = self.backup_profile(
                    profile_name, 'emergency', force=True
                )
                if not emergency_backup.success:
                    logger.warning(
                        f"Could not create emergency backup before restore"
                    )
        ...
        try:
            ...
            for profile in profiles:
                profile_backups = [
                    ...
                    if b['profile_name'] == profile and b['success']
                ]
    ...
    def _is_recent_backup_available(
        ...
        for backup in self._backup_history:
            if (backup['profile_name'] == profile_name and
                backup['backup_type'] == backup_type and
                backup['success']):
    ...
    def _get_latest_backup_metadata(
        self, profile_name: str, backup_type: str
    ) -> BackupMetadata:
        """Get metadata for latest backup"""
        matching_backups = [
            b for b in self._backup_history
            if (b['profile_name'] == profile_name and
                b['backup_type'] == backup_type and
                b['success'])
        ]
...
def emergency_backup(profile_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            'success': False,
            'message': f'Emergency backup failed: {e}',
            'backup_path': ''
        }
