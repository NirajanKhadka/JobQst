...
try:
    from .ai_industry_standards import get_ai_industry_standards
...
class IndustryStandardsDatabase:
    ...
    def validate_with_confidence(self, item_type: str, value: str, context: str = "") -> dict:
        """
        ...
        if item_type == "job_title":
            ...
            ai_validator = self.ai_standards.validate_job_title if self.ai_standards else None
            partial_matcher = self._partial_match_job_title
        elif item_type == "skill":
            standards_set = self.skills
            ai_validator = self.ai_standards.validate_skill if self.ai_standards else None
            partial_matcher = self._partial_match_skill
        elif item_type == "company":
            standards_set = self.companies
            ai_validator = self.ai_standards.validate_company if self.ai_standards else None
        ...
        if self.use_ai and ai_validator:
            try:
                ...
                logger.error(f"AI validation error for {item_type} " f"'{value}': {e}")
    ...
    def find_partial_job_title_match(
        self, title: str, min_similarity: float = 0.6
    ...
    def find_partial_skill_matches(
