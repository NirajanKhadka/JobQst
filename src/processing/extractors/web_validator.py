...
class WebValidator:
    ...
    def _check_search_availability(self) -> bool:
        ...
        if not hasattr(self.search_client, "search"):
            self.logger.warning("WebValidator: Provided search client lacks 'search' method")
            return False
        return True
    ...
    def validate_company(self, company: str) -> ValidationResult:
        ...
        if not self.search_available:
            self.logger.info(f"WebValidator: Web search not available for company '{company}'")
        ...
        try:
            ...
            if results and len(results) > 0:
                self.logger.info(f"WebValidator: Company '{company}' found in search results")
                return ValidationResult(
                    is_valid=True,
                    confidence=0.9,
                ...
                self.logger.warning(f"WebValidator: Company '{company}' not found in search")
                return ValidationResult(
                    is_valid=False,
                    confidence=0.5,
                    validation_method="web_search",
            ...
            self.logger.error(f"WebValidator: Search failed for company '{company}': {e}")
            return ValidationResult(
                is_valid=True,
                confidence=0.5,
    ...
    def validate_salary(self, salary: str) -> ValidationResult:
        ...
        is_valid = any(re.search(pattern, salary, re.IGNORECASE) for pattern in salary_patterns)
