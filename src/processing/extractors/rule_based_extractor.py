...
class RuleBasedExtractor:
    ...
    def _extract_all_fields(
        self, content: str, job_data: Dict[str, Any], result: RuleBasedExtractionResult
    ) -> None:
    ...
    def _validate_extraction(self, result: RuleBasedExtractionResult) -> None:
        ...
        result.field_confidences = {
            ...
            "company": self._get_field_confidence(result.company, self._validate_company_name),
            "location": self._get_field_confidence(result.location, self._validate_location),
            "salary_range": self._get_field_confidence(result.salary_range, self._validate_salary),
            "skills": 0.7 if result.skills else 0.0,
        }

    def _get_field_confidence(self, value: Optional[str], validator_func) -> float:
