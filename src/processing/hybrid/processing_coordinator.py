...
from ..extractors import RuleBasedExtractor, RuleBasedExtractionResult, get_rule_based_extractor
...
class JobProcessingCoordinator:
    ...
    def process_job(self, job_data: Dict[str, Any]) -> ProcessingResult:
        ...
        try:
            if self.strategy == ProcessingStrategy.RULE_BASED_ONLY:
                ...
                self.logger.warning(
                    f"Strategy {self.strategy} not fully implemented, using rule-based"
                )
                return self._process_rule_based_only(job_data, start_time)
    ...
    def _process_rule_based_only(
        self, job_data: Dict[str, Any], start_time: float
    ) -> ProcessingResult:
    ...
    def _process_hybrid_fallback(
        ...
        if extraction_result.overall_confidence >= self.confidence_threshold:
            ...
            self.logger.info(
                f"Rule-based confidence {extraction_result.overall_confidence:.2f} "
    ...
    def process_batch(self, jobs: List[Dict[str, Any]]) -> List[ProcessingResult]:
        ...
        self.logger.info(
            f"Batch processing completed in {batch_time:.2f}s " f"(avg: {avg_time:.3f}s per job)"
        )
...
def process_single_job(
    job_data: Dict[str, Any], strategy: ProcessingStrategy = ProcessingStrategy.RULE_BASED_ONLY
) -> ProcessingResult:
