...
def display_job_cards(df: pd.DataFrame, limit: int = 6) -> None:
    """Display job cards with modern styling."""
    if df.empty:
    ...
    for idx, (_, job) in enumerate(recent_jobs.iterrows()):
        ...
        with col:
            # Determine status and priority badges
            priority = job.get('priority', 'Medium')
            status_text = job.get('status_text', 'New')
            status_stage = job.get('status_stage', 0)
            
            priority_class = f"{priority.lower()}-priority" if priority else "medium-priority"
            
            # Status badge class based on pipeline stage
            status_class_map = {
                'New': 'new-status',
                'Scraped': 'scraped-status', 
                'Processed': 'processed-status',
                'Document Created': 'document-status',
                'Applied': 'applied-status'
            }
            status_class = status_class_map.get(status_text, 'new-status')
            ...
            stage_emoji = stage_emoji_map.get(status_text, '⭕')
            ...
            action_buttons += f'''
                <button onclick="navigator.clipboard.writeText('{title} at {company}')" style="
                    padding: 0.5rem 1rem;
                    background: var(--bg-secondary);
                    color: var(--text-primary);
                    border: 1px solid var(--border-color);
            ...
            st.markdown(
                ...
                <div class="job-card fade-in">
                    ...
                    <div class="job-meta">
                        ...
                        <span class="job-badge {priority_class}">{priority} Priority</span>
        ...
        with st.spinner("Processing application..."):
            ...
            if "Manual" in mode:
                ...
                try:
                    ...
                    st.warning("⚠️ Applier module not available. Falling back to manual mode.")
...
def display_jobs_table(df: pd.DataFrame, profile_name: str = None) -> None:
    ...
    if not filtered_df.empty:
        ...
        # Format URLs as clickable links and add Apply buttons
        if 'url' in display_df.columns:
            ...
            for idx, row in display_df.iterrows():
                ...
                if status == 'applied':
                    ...
                    button_key = f"apply_{idx}_{hash(str(job_url))}"
                    apply_buttons.append(f'🎯 Apply_{button_key}')
