...
def register_processing_callbacks(app):
    ...
    def quick_process(start_clicks, profile_data):
        ...
        try:
            ...
            cmd = [_sys.executable, str(main_py), profile_name,
                   '--action', 'analyze-jobs']
            ...
            if result.returncode == 0:
                return {
                    'message': (f"Processing complete for {profile_name}.\n"
                                f"Output:\n{result.stdout[-1000:]}")
                }
            return {
                'message': (f"Processing failed (code {result.returncode}).\n"
                            f"Error:\n{result.stderr[-1000:]}")
            }
    ...
    def update_processing_status(n_intervals):
        ...
        try:
            ...
            profile_name = getattr(app, 'current_profile', 'Nirajan')
            ...
            return (queue_status, processing_status,
    ...
    def handle_processing_actions(start_clicks, pause_clicks, stop_clicks,
                                  n_intervals, method, batch_size,
                                  max_jobs, options, quick_log_store):
        ...
        try:
            ...
            if button_id == 'start-processing-btn' and start_clicks:
                ...
                return _start_processing(method, batch_size, max_jobs,
                                         options)
            ...
            if _processing_state['is_running']:
                ...
                if _processing_state['jobs_total'] > 0:
                    progress = int((_processing_state['jobs_processed'] /
                                    _processing_state['jobs_total']) * 100)
    ...
    @app.callback(
        Output('processing-method', 'value'),
...
def _pause_processing():
    ...
    current_progress = 0
    if _processing_state['jobs_total'] > 0:
        current_progress = int((_processing_state['jobs_processed'] /
                                _processing_state['jobs_total']) * 100)
