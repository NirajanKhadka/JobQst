...
def apply_job_filters(df, search_term, company_filter, status_filter, start_date, end_date):
    ...
    if start_date and end_date:
        ...
        filtered_df = filtered_df[
            (filtered_df["created_at"] >= start_date) & (filtered_df["created_at"] <= end_date)
        ]
...
def calculate_job_metrics(df):
    ...
    return (f"{total_jobs:,}", f"{new_jobs:,}", f"{ready_jobs:,}", f"{applied_jobs:,}")
...
def register_jobs_callbacks(app):
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.warning("DataLoader not available, using sample data")
                    return [
                        {
                            "id": 1,
                        ...
                        },
                        {
                            ...
                            "job_url": ("https://example.com/jobs/" "data-scientist-2"),
                        },
                        {
                            "id": 3,
                            ...
                            "job_url": ("https://example.com/jobs/" "frontend-developer-3"),
                        },
                    ]
    ...
    def update_jobs_table(
        jobs_data, search_term, company_filter, status_filter, start_date, end_date
    ):
        ...
        try:
            ...
            filtered_df = apply_job_filters(
                df, search_term, company_filter, status_filter, start_date, end_date
            )
    ...
    @app.callback(Output("jobs-company-filter", "options"), Input("jobs-data-store", "data"))
    def update_company_filter_options(jobs_data):
        ...
        try:
            ...
            options.extend([{"label": company, "value": company} for company in companies])
    ...
    @app.callback(
        ...
        [Input("table-view-btn", "n_clicks"), Input("card-view-btn", "n_clicks")],
    ...
    @app.callback(Output("jobs-cards-container", "children"), Input("jobs-data-store", "data"))
    def update_jobs_cards(jobs_data):
        ...
        try:
            ...
            for job in jobs_data[:12]:  # Show first 12 jobs
                cards.append(dbc.Col(create_job_card(job), width=6, lg=4, className="mb-3"))
    ...
    def update_job_tracking_content(active_tab, jobs_data):
        ...
        return html.Div(
            [
                ...
                html.P(
                    f"Status breakdown: {len(filtered_df)} out of " f"{len(df)} total jobs",
    ...
    def export_jobs_to_csv(export_clicks, table_data, filtered_data, profile_data):
        ...
        try:
            ...
            try:
                ...
                profile_name = profile_data.get("name", "jobs") if profile_data else "jobs"
                filename = f"JobQst_{profile_name}_export_{timestamp}.csv"

                return {"content": csv_string, "filename": filename, "type": "text/csv"}
    ...
    # Register smart filter callbacks
    register_smart_filter_callbacks(app)


def register_smart_filter_callbacks(app):
    """Register smart filter button callbacks"""

    @app.callback(
        [
            Output("jobs-status-filter", "value"),
            Output("jobs-search", "value"),
            Output("work-type-filter", "value"),
        ],
        [
            Input("filter-high-match", "n_clicks"),
            Input("filter-ready-apply", "n_clicks"),
            Input("filter-remote", "n_clicks"),
            Input("filter-rcip", "n_clicks"),
            Input("filter-python", "n_clicks"),
            Input("filter-fullstack", "n_clicks"),
            Input("filter-clear-all", "n_clicks"),
        ],
        prevent_initial_call=True,
    )
    def handle_smart_filters(
        high_match, ready_apply, remote, rcip, python_filter, fullstack, clear_all
    ):
        """Handle smart filter button clicks"""
        ctx = callback_context
        if not ctx.triggered:
        ...
        # Clear all filters
        if trigger_id == "filter-clear-all":
            return "all", "", "all"

        # High match jobs (80%+)
        elif trigger_id == "filter-high-match":
            return "all", "", "all"  # We'll handle match score in table filtering

        # Ready to apply jobs
        elif trigger_id == "filter-ready-apply":
            return "ready_to_apply", "", "all"

        # Remote jobs
        elif trigger_id == "filter-remote":
            return "all", "remote", "remote"

        # RCIP cities
        elif trigger_id == "filter-rcip":
            return "all", "RCIP", "all"

        # Python jobs
        elif trigger_id == "filter-python":
            return "all", "python", "all"

        # Full stack jobs
        elif trigger_id == "filter-fullstack":
            return "all", "full stack", "all"
