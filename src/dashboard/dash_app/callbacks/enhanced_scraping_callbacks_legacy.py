...
def register_scraping_callbacks(app):
    ...
    def handle_enhanced_scraping_actions(ultra_fast_clicks, simple_clicks,
        ...
        # Get profile
        profile_name = (profile_data.get('selected_profile', 'Nirajan')
...
def handle_integrated_scraping(trigger_id: str, profile_name: str,
    ...
    if trigger_id in ['start-ultra-fast-pipeline-btn', 'start-simple-scrape-btn',
        ...
        log_entry = (f"[{timestamp}] Started {mode} scraping "
                    f"with {max_jobs} jobs target")
        ...
        return (
            ("\n".join(_scraping_logs[-10:])
             if _scraping_logs else "No recent activity"),
...
def run_integrated_scraping(profile: dict, mode: str,
                           max_jobs: int, sites: list):
    ...
    try:
        ...
        def dashboard_callback(event):
            ...
            if event_type == "scraping_started":
                log_msg = (f"[{timestamp}] 🚀 Starting "
                          f"{event['data']['mode']} pipeline")
        ...
        if success:
            _scraping_logs.append(
                f"[{timestamp}] 🎉 Scraping session completed successfully!")
        else:
            _scraping_logs.append(
                f"[{timestamp}] ⚠️ Scraping session completed with issues")
...
def handle_fallback_scraping(trigger_id: str, profile_name: str,
    ...
    if trigger_id.startswith('start-'):
        return (
            ...
            f"Target: {max_jobs}",
            dbc.Alert("Using fallback mode - integration unavailable",
                     color="warning"),
