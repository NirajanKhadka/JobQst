...
def register_system_callbacks(app):
        ...
        try:
            ...
            try:
                ...
                if cache_dir.exists():
                    ...
                    html_files = len([f for f in os.listdir(html_dir) if f.endswith(".html")])
                    if html_files < 10:
            ...
            try:
                ...
                db = get_job_db(app.profile_name)
                jobs = db.get_jobs()
    ...
    @app.callback(
        Output("service-status-list", "children"), Input("auto-refresh-interval", "n_intervals")
    )
    def update_service_status(n_intervals):
        ...
        try:
            ...
            try:
                ...
                processed_jobs = sum(1 for job in jobs if job.get("final_score") is not None)
                total_jobs = len(jobs)
            ...
            return [create_service_status(name, status) for name, status in services]
    ...
    @app.callback(
        ...
        [Input("log-file-selector", "value"), Input("auto-refresh-interval", "n_intervals")],
    )
