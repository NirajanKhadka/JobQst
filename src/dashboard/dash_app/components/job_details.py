...
def create_skills_breakdown(description, required_skills=None):
    ...
    if not description:
        return html.Div("No skills information available",
                        className="text-muted")
    
    # Common technical skills to look for
    common_skills = [
        'Python', 'JavaScript', 'Java', 'C++', 'C#', 'Ruby', 'PHP', 'Go',
        'Swift', 'React', 'Angular', 'Vue', 'Node.js', 'Django', 'Flask',
        'Spring', 'SQL', 'PostgreSQL', 'MySQL', 'MongoDB', 'Redis',
    ...
    if not found_skills:
        return html.Div(
            "Skills to be determined from job description",
            className="text-muted"
        )
...
def create_company_info_panel(company_name, location=None):
    ...
    return dbc.Card([
        dbc.CardBody([
            ...
            html.P([
                html.Strong("Size: "),
                html.Span(company_size, className="text-muted")
            ], className="mb-1"),
            html.P([
...
def create_work_arrangement_display(location, description=""):
    ...
    # Determine work type
    if any(term in location_lower or term in desc_lower 
           for term in ['remote', 'work from home', 'wfh', 'telecommute']):
        work_type = "Remote"
        icon = "fas fa-home"
        color = "success"
    elif any(term in location_lower or term in desc_lower 
    ...
    return dbc.Badge([
        ...
        work_type
    ], color=color, className="work-arrangement-badge")
...
def create_enhanced_job_modal():
    ...
    return dbc.Modal([
        dbc.ModalHeader([
            dbc.ModalTitle("Job Details", id="enhanced-job-modal-title")
        ]),
        dbc.ModalBody([
            ...
            html.Div(id="job-modal-description-display", 
        ...
        dbc.ModalFooter([
            ...
            dbc.Button(
                "Close",
                id="enhanced-job-modal-close",
                color="secondary"
            )
        ])
    ], id="enhanced-job-modal", size="xl", is_open=False)
