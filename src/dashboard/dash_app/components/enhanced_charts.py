"""
Enhanced chart components for JobLens Dashboard
...
def create_job_funnel_chart(jobs_data):
    ...
    try:
        ...
        funnel_stages = [(stage, count) for stage, count in funnel_stages
                         if count > 0]
...
def create_success_rate_gauge(jobs_data):
    ...
    try:
        ...
        success_rate = ((applied_jobs / total_jobs * 100)
...
def create_match_score_radar(jobs_data):
    ...
    try:
        ...
        very_good = len(df[(df['match_score'] >= 80) & 
                          (df['match_score'] < 90)])
        ...
        fig.update_layout(
            polar=dict(
                radialaxis=dict(
                    visible=True,
                    range=[0, max(values) * 1.1] if max(values) > 0 else [0, 10]
                )),
            showlegend=True,
...
def create_activity_heatmap(jobs_data):
    ...
    try:
        ...
        activity_matrix = df.groupby(['day', 'hour']).size().unstack(
            fill_value=0)
...
def create_performance_metrics_chart(jobs_data):
    ...
    try:
        ...
        metrics = {
            ...
            'High Match (>80%)': len(df[df['match_score'] > 80]) if 'match_score' in df.columns else 0,
