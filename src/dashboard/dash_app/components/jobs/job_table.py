...
def create_jobs_table(data=None):
    ...
    columns = [
        ...
        {"name": "🍁 RCIP", "id": "rcip_badge", "type": "text", "presentation": "markdown"},
...
def create_table_controls():
    ...
    return dbc.ButtonGroup(
        [
            dbc.Button(
                [html.I(className="fas fa-table me-2"), "Table View"],
                id="table-view-btn",
                color="primary",
                size="sm",
                active=True,
            ...
            dbc.Button(
                [html.I(className="fas fa-th-large me-2"), "Card View"],
                id="card-view-btn",
            ...
            dbc.Button(
                [html.I(className="fas fa-filter me-2"), "Advanced Filter"],
...
def format_job_data_for_table(df):
    ...
    # Format confidence scores based on fit_score
    if "fit_score" in display_df.columns:
        ...
        display_df["confidence_badge"] = display_df["fit_score"].apply(
            format_confidence_badge
        )
    elif "confidence_score" in display_df.columns:
    ...
    # Format RCIP badges
    def format_rcip_badge(row):
        badges = []
        if row.get("is_rcip_city"):
            badges.append("🍁 RCIP")
        if row.get("is_immigration_priority"):
            badges.append("⭐ Priority")

        # Add city tags if available
        city_tags = str(row.get("city_tags", ""))
        if city_tags and city_tags.strip():
            for tag in city_tags.split(","):
                tag = tag.strip()
                if tag and tag not in ["rcip", ""]:
                    badges.append(f"🏷️ {tag.title()}")

        return " | ".join(badges) if badges else "—"

    display_df["rcip_badge"] = display_df.apply(format_rcip_badge, axis=1)
    ...
    if "actions" not in display_df.columns:
        ...
        def create_action_buttons(row):
            ...
            if job_url and job_url != "#" and job_url != "None":
                ...
                view_btn = '<span class="btn btn-sm btn-secondary disabled">' "👁️ View</span>"
            ...
            summary_btn = (
                f'<button class="btn btn-sm btn-info job-summary-btn" '
