...
from src.dashboard.dash_app.components.job_table import create_jobs_table, create_table_controls
from src.dashboard.dash_app.components.job_cards import create_jobs_grid, create_job_quick_stats
from src.dashboard.dash_app.components.job_modal import (
    create_job_modal,
        ...
        [
            ...
            dbc.ModalBody(
                [
                    ...
                    dcc.Textarea(id="notes-modal-text", style={"width": "100%", "height": 180}),
                    html.Small(
            ...
            dbc.ModalFooter([dbc.Button("Close", id="notes-modal-close", color="secondary")]),
        ],
        size="sm",
        className="mt-3",
    )
...
def create_jobs_header():
    ...
    header_actions = dbc.ButtonGroup(
        [
            dbc.Button(
                [html.I(className="fas fa-plus me-1"), "Add Job"], color="primary", size="sm"
            ),
            dbc.Button(
    ...
    return create_page_header(
        "💼 Job Management", "Manage and track your job applications", header_actions
    )
    ...
    return dbc.Row(
        [
            dbc.Col(
                [
                    create_metric_card(
                        "Total Jobs", "0", "fas fa-briefcase", "primary", "total-jobs-metric"
                    )
                ],
                width=3,
            ),
            dbc.Col(
                [
                    create_metric_card(
                        "New Jobs", "0", "fas fa-plus-circle", "info", "new-jobs-metric"
                    )
                ],
                width=3,
            ),
            dbc.Col(
                [
                    create_metric_card(
                        "Ready to Apply", "0", "fas fa-paper-plane", "success", "ready-jobs-metric"
                    )
                ],
                width=3,
            ),
            dbc.Col(
                [
                    create_metric_card(
                        "Applied", "0", "fas fa-check-circle", "warning", "applied-jobs-metric"
                    )
                ],
                width=3,
            ),
...
def create_smart_filter_buttons():
    """Create smart filter buttons for quick job discovery"""
    return html.Div(
        [
            html.Label("🚀 Smart Filters", className="fw-semibold mb-2"),
            dbc.Row(
                [
                    dbc.Col(
                        [
                            dbc.ButtonGroup(
                                [
                                    dbc.Button(
                                        [html.I(className="fas fa-star me-1"), "High Match (80%+)"],
                                    ...
                                    dbc.Button(
                                        [html.I(className="fas fa-bolt me-1"), "Ready to Apply"],
                                        id="filter-ready-apply",
                                        color="primary",
                                        outline=True,
                                        size="sm",
                                    ),
                                    dbc.Button(
                                        [html.I(className="fas fa-home me-1"), "Remote Jobs"],
                                        id="filter-remote",
                                        color="info",
                                        outline=True,
                                        size="sm",
                                    ...
                                    dbc.Button(
                                        [html.I(className="fas fa-flag me-1"), "🇨🇦 RCIP Cities"],
                                        id="filter-rcip",
                                        color="warning",
                                        outline=True,
                                        size="sm",
                    ...
                    dbc.Col(
                        [
                            dbc.ButtonGroup(
                                [
                                    dbc.Button(
                                        [html.I(className="fas fa-code me-1"), "Python Jobs"],
                                        id="filter-python",
                                        color="secondary",
                                        outline=True,
                                        size="sm",
                                    ...
                                    dbc.Button(
                                        [html.I(className="fas fa-layers me-1"), "Full Stack"],
                                        id="filter-fullstack",
                                        color="secondary",
                                        outline=True,
                                        size="sm",
                                    ...
                                    dbc.Button(
                                        [html.I(className="fas fa-refresh me-1"), "Clear All"],
                                        id="filter-clear-all",
                                        color="light",
                                        outline=True,
                                        size="sm",
                            ...
                            dbc.Col(
                                [
                                    html.Div(
                                        [
                                            html.Small(
                                                "Quick Filters: ", className="text-muted me-2"
                                            ),
                                            dbc.Badge(
                                                "Remote Only",
                                                color="info",
...
def create_jobs_table_section():
    ...
    return dbc.Card(
        [
            dbc.CardHeader([create_table_header()]),
            dbc.CardBody([create_table_view(), create_card_view()]),
        ]
    )
...
def create_table_header():
    ...
    return dbc.Row(
        [
            dbc.Col([html.H5("📋 Job Listings", className="mb-0")], width="auto"),
            dbc.Col(
                [
                    dbc.ButtonGroup(
                        [
                            dbc.Button(
                                [html.I(className="fas fa-table me-1"), "Table"],
                                color="primary",
                                size="sm",
                                id="table-view-btn",
                                active=True,
                            ...
                            dbc.Button(
                                [html.I(className="fas fa-th-large me-1"), "Cards"],
        ...
        [
            dash_table.DataTable(
                ...
                columns=[
                    ...
                    {"name": "View Job", "id": "view_job", "presentation": "markdown"},
                ],
                data=[],
                sort_action="native",
...
def create_card_view():
    ...
    return html.Div(
        [html.Div(id="jobs-cards-container")], id="card-view", style={"display": "none"}
    )


def get_table_cell_style():
    """Get table cell styling"""
    return {"textAlign": "left", "padding": "12px", "fontFamily": "Inter, sans-serif"}


def get_table_header_style():
    """Get table header styling"""
    return {"backgroundColor": "#f8f9fa", "fontWeight": "bold", "borderBottom": "2px solid #dee2e6"}
                ...
                [
                    dbc.Row(
                        [
                            dbc.Col(
                                [
                                    html.H4(id=metric_id, children=value, className="fw-bold mb-1"),
                        ...
                        [
                            dbc.Col(
                                [
                                    ...
                                    html.H5(job.get("title", "Unknown Position"), className="mb-2"),
                                ],
                                width=8,
                            ),
                            dbc.Col(
