    ...
    try:
        ...
        fig = go.Figure(data=[
            go.Pie(
                ...
                marker=dict(colors=[colors.get(status, '#757575') for status in status_counts.index])
            )
        ])
...
def create_application_funnel(jobs_data):
    ...
    try:
        ...
        valid_stages = [(stage, count) for stage, count in zip(funnel_stages, stage_counts) if count > 0]
...
def create_skill_demand_chart(jobs_data, top_n=15):
    ...
    try:
        ...
        skills_cols = [col for col in df.columns if any(keyword in col.lower() for keyword in ['skill', 'requirement', 'keyword'])]
