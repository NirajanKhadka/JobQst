...
class SalaryParser:
    ...
    def _load_from_config(self):
        """Load salary patterns and currencies from config"""
        try:
            import sys
            ...
            loader = ConfigLoader()
            config = loader.load_config("salary_config.json")
            
            # Load currency symbols - handle nested structure
            currency_config = config.get("currency_symbols", {})
            self.currency_symbols = {}
            
            for symbol, data in currency_config.items():
                if isinstance(data, dict):
                    # New format: {"$": {"code": "CAD", ...}}
                    self.currency_symbols[symbol] = data.get("code", "CAD")
                else:
                    # Old format: {"$": "CAD"}
                    self.currency_symbols[symbol] = data
            
            # Add defaults if empty
            if not self.currency_symbols:
                self.currency_symbols = {
                    "$": "CAD", "€": "EUR", "£": "GBP", "¥": "JPY",
                    "₹": "INR", "A$": "AUD", "C$": "CAD", "US$": "USD"
                }
            
            # Load patterns from config
            config_patterns = config.get("salary_patterns", {})
            if config_patterns:
                self.salary_patterns = self._build_patterns_from_config(config_patterns)
            else:
                self.salary_patterns = self._init_salary_patterns()
                
            logger.info(f"Loaded salary config: {len(self.currency_symbols)} currencies, {len(self.salary_patterns)} pattern groups")
        except Exception as e:
            logger.warning(f"Failed to load salary config: {e}, using defaults")
            self.salary_patterns = self._init_salary_patterns()
            self.currency_symbols = {"$": "CAD", "€": "EUR", "£": "GBP"}
    
    def _build_patterns_from_config(self, config_patterns: Dict[str, Any]) -> Dict[str, list]:
        """Build regex patterns from config"""
        patterns = {}
        
        # Config structure: ranges, single_amount, hourly_rates, etc.
        # Each has {"confidence": X, "weight": "level", "patterns": []}
        
        # Map weight levels to our confidence system
        weight_map = {
            "very_high": "very_high",
            "high": "high",
            "medium": "medium",
        ...
        for pattern_type, pattern_data in config_patterns.items():
            if isinstance(pattern_data, dict) and "patterns" in pattern_data:
                weight = pattern_data.get("weight", "medium")
                confidence_level = weight_map.get(weight, "medium")
                
                if confidence_level not in patterns:
                    patterns[confidence_level] = []
                
                patterns[confidence_level].extend(pattern_data["patterns"])
    ...
    def _init_salary_patterns(self) -> Dict[str, list]:
        ...
        return {
            ...
            "medium": [
                ...
                (r"\$?([\d,]+(?:\.\d{2})?)\s*(?:k|,000)?\s*" r"(?:per\s*year|annually|/year)"),
            ],
            "low": [
                # Hourly rates
    ...
    def parse_salary(self, text: str, job_title: str = "") -> Optional[SalaryInfo]:
        """
        ...
        for confidence_level, patterns in self.salary_patterns.items():
            for pattern in patterns:
                ...
                if match:
                    salary_info = self._process_match(match, text, confidence_level)
    ...
    def _process_match(
        self, match, original_text: str, confidence_level: str
    ) -> Optional[SalaryInfo]:
            ...
            if len(groups) >= 2 and groups[1]:
                ...
                if min_val and max_val:
                    return SalaryInfo(
                        ...
                        max_amount=max_val,
                        is_range=True,
                        confidence=self._get_confidence_score(confidence_level),
    ...
    def _parse_amount(self, amount_str: str) -> Optional[float]:
        ...
        # Clean the string - remove currency symbols first
        clean_amount = amount_str
        for symbol in self.currency_symbols.keys():
            clean_amount = clean_amount.replace(symbol, "")
        
        clean_amount = re.sub(r"[^\d.,km]", "", clean_amount.lower())

        try:
            # Handle 'k' or 'm' suffix (thousands/millions)
            if "k" in clean_amount:
                base_amount = float(clean_amount.replace("k", "").replace(",", ""))
                return base_amount * 1000
            elif "m" in clean_amount:
                base_amount = float(clean_amount.replace("m", "").replace(",", ""))
    ...
    def _validate_salary(self, salary_info: SalaryInfo) -> bool:
        ...
        if (
            salary_info.is_range
            and salary_info.max_amount
            and salary_info.max_amount < salary_info.min_amount
        ):
            return False
    ...
    def _get_confidence_score(self, confidence_level: str) -> float:
        ...
        scores = {"very_high": 0.95, "high": 0.85, "medium": 0.70, "low": 0.50}
...
def get_salary_statistics(jobs_data: list) -> Dict[str, Any]:
    ...
    return {
        ...
        "median_salary": (
            salaries[n // 2] if n % 2 == 1 else (salaries[n // 2 - 1] + salaries[n // 2]) / 2
        ),
    }
