...
def format_job_data_for_table(jobs_data):
    ...
    try:
        ...
        df["actions"] = df.apply(
            lambda row: f"[View]({row.get('url', '#')}) | [Apply](#) | [Notes](#)", axis=1
        )
...
def calculate_job_metrics(jobs_data):
    ...
    try:
        ...
        metrics = {
            ...
            "ready_to_apply": (
                len(df[df["status"] == "ready_to_apply"]) if "status" in df.columns else 0
            ),
            "applied_jobs": len(df[df["status"] == "applied"]) if "status" in df.columns else 0,
            "needs_review": (
                len(df[df["status"] == "needs_review"]) if "status" in df.columns else 0
            ),
            ...
            "remote_jobs": (
                len(df[df["location"].str.contains("Remote", case=False, na=False)])
