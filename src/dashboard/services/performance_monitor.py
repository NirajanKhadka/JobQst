...
class PerformanceMonitor:
    ...
    def get_performance_summary(self, hours_back: int = 24) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            recent_queries = [
                q for q in self.query_metrics if datetime.fromisoformat(q.timestamp) > cutoff
            ]

            recent_system = [
                s for s in self.system_metrics if datetime.fromisoformat(s.timestamp) > cutoff
            ]
            ...
            return {
                ...
                "slow_queries_count": len(
                    [q for q in recent_queries if q.duration_ms > self.slow_query_threshold]
                ),
    ...
    def get_slow_queries(self, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            return [asdict(q) for q in list(self.slow_queries)[-limit:]]

    def get_query_trends(self, query_type: str = None, hours_back: int = 24) -> Dict[str, Any]:
    ...
    def _analyze_queries(self, queries: List[QueryMetrics]) -> Dict[str, Any]:
        ...
        return {
            ...
            "p95_duration_ms": (
                statistics.quantiles(durations, n=20)[18] if len(durations) > 5 else max(durations)
            ),
            "by_type": type_stats,
        }
