...
class SystemHealthChecker:
    ...
    def check_network_health(self) -> bool:
        ...
        if success_rate >= 0.75:  # 75% success rate
            console.print(f"[green]✅ Network health good: {successful_connections}/{len(test_urls)} sites accessible[/green]")
            return True
        else:
            console.print(f"[red]❌ Network health poor: {successful_connections}/{len(test_urls)} sites accessible[/red]")
            return False
    ...
    def check_disk_space(self) -> bool:
        ...
        try:
            ...
            console.print(f"[cyan]💾 Disk Usage: {used_gb:.1f}GB / {total_gb:.1f}GB ({usage_percent:.1f}%)[/cyan]")
            ...
            try:
                ...
                if hasattr(os, 'sysconf') and hasattr(os, 'sysconf_names'):
                    try:
                        ...
                        console.print(f"[cyan]🧠 Total Memory: {total_memory:.0f}MB (basic check)[/cyan]")
    ...
    def check_services(self) -> bool:
        ...
        required_files = [
            "requirements.txt",
            "README.md",
            f"profiles/{self.profile_name}/nirajan_profile.json"
        ]
        ...
        if success_rate >= 0.8:  # 80% success rate
            console.print(f"[green]✅ Services health good: {sum(checks)}/{len(checks)} checks passed[/green]")
            return True
        else:
            console.print(f"[red]❌ Services health poor: {sum(checks)}/{len(checks)} checks passed[/red]")
            return False
    ...
    def run_comprehensive_check(self) -> Dict[str, bool]:
        ...
        console.print(f"\n[bold {'green' if overall_health else 'red'}]Overall System Health: {'HEALTHY' if overall_health else 'ISSUES DETECTED'}[/bold {'green' if overall_health else 'red'}]")
    ...
    def get_health_recommendations(self, results: Dict[str, bool]) -> List[str]:
        ...
        if not results.get("services", True):
            recommendations.append("⚙️ Install missing dependencies with: pip install -r requirements.txt")
